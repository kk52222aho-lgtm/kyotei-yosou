"""群衆が構造的に間違える"別の断面"を探す＝既存エッジを条件でスライスして増幅点を見つける。

今日の展開実験で確定: "予測に情報を足す"道は死(市場が公開情報を全部値付け)。
∴次は"もっと賢い予測"でなく"群衆が情報処理をサボる/間違える断面"を探す。
エッジ本体=群衆thaインを過剰に買う。問い=その過剰さが一番現実からズレるのはどの断面か。
既存の確定エッジ(本命≠1号の2連単3点=約183%)を条件でスライスし、増幅する断面を出す:
  風速/波高(条件=群衆が紙スペックに固執し織込みが遅いsticky prior・荒天ほどイン過剰買い?)
