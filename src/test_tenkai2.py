...
test_tenkaiで展開(6艇相対)が初めて1着精度を+1.2pt動かした。だが妙味単勝は-17pt=落ちた。
仮説: 展開(展示ST/タイム)は客も見る公開情報→モデルが群衆に近づき精度up・逆張りエッジdown
     =市場効率の壁と整合(上がった分は既に値付け済)。
決着=1着精度でなく本物の金軸で測る: 本命≠1号レースの2連単3点ROI(既存183-195%の本戦略)を
     ベース vs +展開 で年別に。展開が本当のエッジ源なら2連単3点も上がるはず。落ちれば
...
def main():
    ...
    if ep - eb >= 8:
        ...
        print("  展開は1着精度を上げるが本物のエッジ(2連単3点)は上げない/むしろ削る")
        print("  ＝仮説どおり『展開は公開情報で市場が既に値付け済、精度up分はエッジにならん』で確定。")
