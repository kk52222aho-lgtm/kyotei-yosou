...
near-lock表はBT(展示込み特徴)の確信で作った。ライブ朝スキャンは展示前→展示特徴が欠落→
確信が別分布の恐れ。∴各テスト年の予測を「展示を潰した入力」(tenji_time/tenji_st→NaN=朝の再現)
で出し直し: ①朝の確信が何%まで届くか(分布) ②朝の確信≥80%が本当に84%当てるか(較正照合)。
...
def main():
    ...
    print("\n判定: 朝が『≥80%帯が十分発火 & 実的中≈84%』なら表を朝に適用可。")
    print("      朝で≥80%が激減 or 的中が84%とズレるなら=分布が別物=表は朝に使えない")
