...
【事前登録(FROZEN・以後この定義を再最適化しない)】
 ...
 採点: 前向き・実オッズ確定settleで N>=200 fires 貯める。合格= racer_idクラスタbootstrapの
  生涯ROI CI下限>=100。機構検品として star/地元/展示-hot vs 無名×モーター の分割も"記録だけ"する
  (ここで閾値を選び直さない)。→ 触らず走らせる。採点者は唯一作文できない未来だけ。
