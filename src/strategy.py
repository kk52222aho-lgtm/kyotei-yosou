...
∴これは"未証明の仮説+妥当な機構"であって勝ちモデルではない。probeとして極小賭けで前向き収集
(150-300 fires)し、実オッズで検証するまで信じない。増資禁止(自己インパクトでedge消滅)。
未実施の潔白化: 閾値を2022/2026で凍結→2023-25適用 / 同一オッズ帯でイン外比較 / racer_id
クラスタbootstrap / 無名×モーター駆動に限定(機構由来の事前登録ルール)。
