...
発見: win IS NOT NULL フィルタが、1号艇が転覆/失格/エンスト/不完走で着順なし→データ落ち
のレースを作る。するとモデルの本命が自動で非1号=「妙味」に誤分類される。これは
...
生き残るか測る。あり側>150%なら本物・罠は限定的。あり側が崩れれば全土台が水増しで確定。
...
def main():
    ...
    print(f"  うち 1号艇が飛んで消えたレース(lane1なし) = {absent:,} ({absent/n*100:.0f}%)  <- ここが問題の核\n")
    ...
    print("\n判定: 1号あり側が>150%で3年一貫なら土台は本物・罠は限定的。")
    print("      1号あり側が100%付近まで落ちれば既存116/188%は飛び艇survivorshipの水増しで確定=最重要。")
