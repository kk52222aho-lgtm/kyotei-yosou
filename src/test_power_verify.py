"""#2=地力(実力)が本物か、それとも"強い選手は2号に入ってるだけ"の言い換えかを潰す。

test_when_inside_failsで地力38.6%>差し2号30.7だったが、今日の教訓=良い数字ほど疑う。
最大の疑い: 地力=ただの2号(差しの好位置)の言い換えではないか。3つの敵対検証:
  1. 位置を固定しても地力が効くか(各非イン枠の中で地力上位ほど勝つか=skill独立性)=本丸
  2. 地力ランク別に単調か(非イン地力1位→5位で勝率が綺麗に下がるか)
  3. 年別で#2安定か
...
def main():
    ...
    print("[検証2: 非イン内の地力ランク別 勝率(単調に下がれば実力が効いてる)]")
    for k in [1, 2, 3, 4, 5]:
        sub = d[d["nrk"] == k]
        print(f"  地力{k}位: 勝率 {wr(sub):4.1f}%  (n{len(sub)})")

    print("\n[検証1(本丸): 枠を固定して地力が効くか＝2号の言い換えでない証明]")
    print("  各枠の中で 地力上位(非イン1-2位) vs 下位(4-5位) の勝率差が出れば実力は位置と独立")
    for ln in [2, 3, 4, 5]:
    ...
    print("\n判定: 検証2が単調・検証1で全枠プラス差・検証3で3年安定 なら #2=地力は本物(位置と独立の実力)。")
    print("      枠固定で差が消えるなら『地力=2号の言い換え』=偽物。確定払戻・walk-forward・6艇クリーン。")
