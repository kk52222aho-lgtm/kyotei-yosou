...
def page_pro():
    ...
    st.header("🎯 プロならどう賭けるか（一日の全レースの捌き方）")
    st.markdown("""
他のページで見せてるのは**モデル（数学）の出力**。でも**プロ舟券師は「全レース買わない」**。
一番の技術は当てることやなくて、**大半を見送って、歪んだ数レースだけ選ぶこと**。
実際の一日で、これを数字で見せる。
""")
    path = os.path.join(os.path.dirname(__file__), "data", "pro_day.json")
    if not os.path.exists(path):
        ...
        nv, pr = d["naive"], d["pro"]
        dd = d["date"]
        st.markdown(f"#### サンプル：{dd[:4]}/{dd[4:6]}/{dd[6:]} の全 {nv['races']} レース")
        c = st.columns(2)
        c[0].markdown(f"**❌ 全レース機械買い**（モデルの本命単勝＋2連単3点を{nv['races']}レース全部）  \n"
                      f"回収 **{nv['roi']*100:.0f}%** ／ 収支 **{nv['pl']:+,}円**  \n"
                      f"単勝的中 {nv['t_hit']}/{nv['races']}・2連単的中 {nv['e_hit']}/{nv['races']}")
        c[1].markdown(f"**⭕ プロの選択**（妙味＝本命≠1号艇 だけ＝{pr['races']}レース）  \n"
                      f"回収 **{pr['roi']*100:.0f}%** ／ 収支 **{pr['pl']:+,}円**  \n"
                      f"**{nv['races']-pr['races']}レースを見送り**（全体の{(1-pr['races']/nv['races'])*100:.0f}%）")
        st.warning("**この日の収支はどちらもノイズ**（一日は誰でもブレる／プロ側は3本＝小さすぎ）。"
                   "肝は金額やなくて『**211レース見送った規律**』と、歪みのある所だけに賭ける『**選択**』。"
                   "本当のエッジ（長期170%）は年単位でしか出ん。")

        st.markdown("##### プロが賭けた妙味レース（この日）")
        tbl = [{"レース": f"{r['venue']}{r['rno']}R",
                "本命": f"{r['honmei']}号", "1着": f"{r['winner']}号",
                "単勝": ("🎯+" + str(r["t_ret"] - 100)) if r["won_t"] else "×-100",
                "2連単": ("🎯 払戻" + str(r["e_ret"])) if r["e_win"] else "×",
                "収支": r["pl"]}
               for r in d["races"] if r["contrarian"]]
        st.dataframe(pd.DataFrame(tbl), hide_index=True, use_container_width=True,
                     column_config={"収支": st.column_config.NumberColumn(format="%+d")})

    st.markdown("""
#### 何を学んだか（この検証を通して）
- **儲けは「当てる」やなくて「選ぶ」**。モデルの本命を全部買っても控除率でトントン以下。
  利益は、公衆が1号艇を過剰人気にして**市場が値付けを間違えた数レース**に集中しとる。
- **足しても伸びなかったもの**（全部試して否定した）：選手個人の癖（生ID）、3連単、モーター/ボートID、特徴量の追加。
  ＝これ以上「予測精度」を上げる余地は薄い。エッジは精度でなく**選択と規律**にある。
- **一日・数十本は必ずノイズ**。2連単は稀な高配当が牽引するファットテール＝勝った負けたに一喜一憂した方が負ける。

#### どこを見るか（三つの関門・上から順に捨てる）
1. **本命≠1号艇か？** モデルの本命が1号艇なら公衆と同じ見立て＝妙味なし＝**見送り**（この日は211レースがこれ）
2. **本命の単勝が1.5倍以上か？** 1.5未満は大本命＝荒れない＝単勝も2連単も控除負け＝**レース丸ごと見送り**
3. **2連単の締切EVが2.0超か？** 確率×締切オッズが2.0を超えた時だけ2連単を買う（市場と十分ズレてる所だけ）

＝214レースを3つの関門でふるいにかけて、残ったごく僅かだけに賭ける。**プロの仕事の9割は「買わない判断」**。
""")
    st.info("※これは研究目的のシステム。実弾は少額(100円)想定・半自動(自分で投票)。"
            "統計的推定であり的中・利益を保証しません。投資は自己責任で。")
